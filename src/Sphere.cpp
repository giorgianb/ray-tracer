...
Sphere::Sphere(const Vector pos, const double radius, const Material material): 
...
MaybeVector Sphere::transmit(const Ray& ray) const {
	const double dx {ray.direction().x()};
	const double dy {ray.direction().y()};
	const double dz {ray.direction().z()};
...
Material Sphere::material(const Vector& point) const {
	assert(std::fabs(magnitude(point - _pos) - _radius) < 0.001);
